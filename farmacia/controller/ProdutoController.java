...
public class ProdutoController {
	...
	public ResponseEntity<ProdutoModel> put(@Valid @RequestBody ProdutoModel produtoModel){
		return produtoRepository.findById(produtoModel.getId()).map(resposta -> ResponseEntity.status(HttpStatus.OK).body(produtoRepository.save(produtoModel))).orElse(ResponseEntity.status(HttpStatus.NOT_FOUND).build());
	}
